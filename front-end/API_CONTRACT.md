...
| `solve_collinear` | `{"target": "SEW_3p1", "rhs": "boundary_2L.wxf", "projection": "finite", "letter_projection": "identity", "pair_letters": "identity, output/collinear/colprojdiv_w1.wxf", "export_conditions": true, "out_stem": "nmhv2pairs", "solver": "incremental"}` | inputs `seed` (optional), `rhs` (optional), `cond` (optional, kind matrix/tensor: rank-2 `[M\|r]` conditions file), plus dynamic pairs `in_seed_N`/`in_rhs_N` (N ≥ 1, kind seed_or_tensor/boundary; one spare pair shown on canvas); outputs `solution` and `conditions` (kind matrix, `[M\|r]`). Without a wired seed: `target` + `projection` `finite\|divergent` run the named-target contract (`--target SEW_FpL --projection finite`). With a wired seed that is not a fec/sew output: custom-seed mode — `--target-basis <file> --projection none`, no naming convention (any tensor computed upstream in the flow; `projection` forced to `none`). With a wired fec/sew seed: the wired value overrides `target`. The wired `rhs` port overrides the `rhs` field (`0` = all-zero RHS); accepted wired kinds: boundary/tensor/basis/solution/matrix (alphabet `precomputed_tensor` outputs are kind `matrix`, so E1.wxf can be wired directly from an Alphabet node). **Multi-pair mode** (triggered by wiring any `in_seed_N` or the `cond` port, or by `export_conditions` with a custom seed): each pair `{seed, rhs, letter projection}` contributes its own non-homogeneous constraints and all rows are stacked into ONE `solve_linear_system_incremental` call (`--pair <seed> <rhs\|0> <letter\|identity>` per pair); every seed is used as-is (custom-seed semantics, no seed-space projection). Per-pair letter projections come from `pair_letters` (comma-separated; entry 0 = pair 0 and overrides `letter_projection`, entry N = seed N/rhs N; default `identity`). Unwired rhs N defaults to `0` (homogeneous constraints). Wired `cond` files (`--pair-cond`) stack additional `[M\|r]` rows; re-ingest `conditions` outputs from this or other flows. `out_stem` overrides sol_/cond_ naming (auto: single source stem, multiple `stem1_xN`; derived from cond stems when cond-only). `export_conditions` (or wiring the `conditions` output) writes `output/collinear/cond_<stem>.wxf` = combined `[M\|r]` (rhs = last column). Compile errors: named-target (`target` set, seed unwired) cannot use multi-pair/cond; extra pairs require pair-0 seed; cond-only ignores Target/rhs ports |

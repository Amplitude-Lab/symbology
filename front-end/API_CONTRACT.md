...
| `solve_collinear` | `{"target": "SEW_3p1", "rhs": "boundary_2L.wxf", "projection": "finite", "letter_projection": "identity", "solver": "incremental"}` | inputs `seed` (optional), `rhs` (optional); output `solution`. Without a wired seed: `target` + `projection` `finite\|divergent` run the named-target contract (`--target SEW_FpL --projection finite`). With a wired seed that is not a fec/sew output: custom-seed mode — `--target-basis <file> --projection none`, no naming convention (any tensor computed upstream in the flow; `projection` forced to `none`). With a wired fec/sew seed: the wired value overrides `target`. The wired `rhs` port overrides the `rhs` field (`0` = all-zero RHS); accepted wired kinds: boundary/tensor/basis/solution/matrix (alphabet `precomputed_tensor` outputs are kind `matrix`, so E1.wxf can be wired directly from an Alphabet node) |

...
def compile_flow(proj: dict, graph: dict, output_subdir: str | None = None) -> dict:
    ...
    if ".." in Path(subdir).parts or Path(subdir).is_absolute():
        return {"ok": False, "errors": ["Output subdirectory must stay inside output/."], "steps": []}
    ...
    def add_step(label, kind, argv, cwd, outputs, skip_if_exists, meta=None):
        inputs, input_dirs = execution.input_manifest([str(a) for a in argv], str(cwd or proj_dir), outputs)
        steps.append({
            "inputs": inputs,
    ...
    for step in steps:
        for output in step["outputs"]:
            if output in owners and owners[output] != step["argv"]:
                errors.append(f"Multiple steps write '{output}'; choose distinct targets.")
            owners[output] = step["argv"]
    if errors:
        return {"ok": False, "errors": errors, "steps": []}
...
def export_flow_script(proj: dict, graph: dict, flow_name: str, output_subdir: str | None = None) -> dict:
    ...
    proj_dir = storage.project_dir(proj["id"])
    payload = dict(project=str(proj_dir), repo=str(REPO_ROOT),
                   wolfram=find_wolframscript() or "wolframscript", steps=steps)
    for step in steps:
        if step["kind"] == "wolfram":
            step["precomputed"] = dict(
                verified=execution.step_cached(step, proj_dir),
                inputs={str(p): execution.digest(p) if p.is_file() else "missing"
                        for p in execution.input_files(step, proj_dir)
                        if str(p) != step["argv"][0]},
                outputs={o: execution.digest(proj_dir / o) for o in step.get("outputs", [])
                         if (proj_dir / o).is_file()})
    ...
    return {"ok": True, "path": str(script_path), "n_steps": len(steps),
            ...
            "flow_outputs": sorted({f["file"] for f in result.get("flow_outputs", []) if f.get("file")})}
...
def _compile_alphabet(proj, node, provides, add_step, errors, wolframscript, gen_dir, proj_dir, tensor_ops_bin=None, file_steps=None):
    ...
    for prop_id in selected:
        ...
        if prop.get("definition_stale") and (prop.get("precomputed") or prop["type"] == "precomputed_tensor"):
            errors.append(f"Alphabet '{alphabet['name']}': '{property_display_name(prop)}' belongs to an older definition; replace this precomputed property.")
            continue
        if prop.get("status") != "ready" or not (proj_dir / rel).exists():
            ...
            add_step(
                ...
                [rel],
                True,
                {"type": "property", "alphabet_id": alphabet["id"], "property_id": prop_id, "tensor_file": rel, "alphabet_version": storage.alphabet_version(alphabet)},
            )
        base_info = {
            "kind": kind,
            "file": rel,
...
def _infer_weight(rel: str):
    stem = Path(rel).stem
    m = re.match(r"^(?:FEC_|LEC_|FEC|LEC|first_w|last_w)(\d+)$", stem, re.IGNORECASE)
    if m:
...
def _compile_compute_rhs(node, incoming, provides, add_step, errors, tensor_ops_bin, tensor_add_bin, proj_dir):
    ...
    mhv_boundary:  master equation  1 + E = exp_⊗(Σ_k R_k)  (shuffle exp)
      ...
      and ground-truth boundary tensors to L = 4. The native compute_rhs
      implementation also uses this recurrence; the historical L=5 closed
